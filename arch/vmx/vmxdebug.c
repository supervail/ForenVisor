...
* 2011.1     Miao Yu     	Reorganize it for Vis hypervisor on x86 and x86_64(not finished).
					Modify it to make it compatible with Win7
* 
*****************************/
...
)
{
  ...
  addr = IDT_VECTORING_INFO;
  Print (("IDT_VECTORING_INFO 0x%X: 0x%llx\n", addr, VmxRead (addr)));
  ...
  addr = HOST_SYSENTER_CS;
  Print (("HOST_SYSENTER_CS 0x%X: 0x%llx\n", addr, VmxRead (addr)));
  ...
  addr = HOST_SYSENTER_ESP;
  Print (("HOST_SYSENTER_ESP 0x%X: 0x%llx\n", addr, VmxRead (addr)));
  addr = HOST_SYSENTER_EIP;
  Print (("HOST_SYSENTER_EIP 0x%X: 0x%llx\n", addr, VmxRead (addr)));
  addr = HOST_RSP;
